...
describe User do
  ...
  describe "#booked_memberships" do
    ...
    let(:first_memb) { create(:membership, starts_at: Time.now,
      ends_at: nil, user: subject, project: project_current) }
    let(:sec_memb) { create(:membership, starts_at: 1.month.from_now,
      ends_at: nil, user: subject, project: project_next) }
