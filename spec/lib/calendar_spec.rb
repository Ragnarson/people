...
describe Calendar do
  ...
  before do
    ...
    stub_request(:get, "https://www.googleapis.com/discovery/v1/apis/calendar/v3/rest").
      to_return(:status => 200, :body => File.read(File.join('spec', 'support', 'calendar', 'rest.txt')),
  ...
  describe '#import_vacation' do
    ...
    context 'when user has vacation in database' do
      ...
      it 'saves imported event' do
        Timecop.freeze(Time.local(2015, 2, 20)) do
  ...
  describe '#delete_vacation' do
    it 'removed vacation via api' do
      request = stub_request(:delete, "https://www.googleapis.com/calendar/v3/calendars/1/events/1").
        with(:headers => {'Accept'=>'*/*', 'Accept-Encoding'=>'gzip;q=1.0,deflate;q=0.6,identity;q=0.3',
          'Authorization'=>'Bearer 123', 'Cache-Control'=>'no-store',
          'Content-Type'=>'application/x-www-form-urlencoded'}).
        to_return(:status => 200, :body => "", :headers => {})
